...
- **Descriptive layers never score**: `comp_identity`, `kill_pressure`, `fight_chain`, `pick_report`, and the killboard surfaces describe — golden T23c/T25b/T26b/T30d literally prove fitness is untouched by computing them. The one sanctioned influence is the suggestion gate (bars suggestion POOLS, never scoring — forge F6/F13 pin the contract). Identity-aware *scoring* stays parked until more blind rounds validate the labels.
