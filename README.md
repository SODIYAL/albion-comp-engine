...
- `CLAUDE.md` — the contributor and tooling guide: environment traps, the gate list, the build chain, the architecture and the invariants a change must not break.
- `tests/VALIDATION.md` — the decision log: the standing rules and one index row per decision, with the full dated history in `notes/validation/`.
...
**Units:** every "how much a team needs" number speaks the unit the engine measures — whole *people* (weapon plus armour, cape and consumables). The template rows were re-fitted together, never piecemeal, and structural hard floors deliberately stay in weapon units (see `CLAUDE.md`, "One unit, everywhere").

## Observed evidence and caller tools
...
Built through blind validation rounds with experienced callers, all descriptive — none of it moves a score:

- **Weapon identity**: every weapon carries a derived style/size fit — its E spell first ("the E is the weapon's identity"), then delivery reach, damage scale, and utility — with cited fact overrides (`pipeline/style_overrides.yaml`) and a full audit report. A melee weapon whose E lands at range (Realmbreaker) derives as the all-rounder; a weapon whose E can't carry group damage (Battleaxe) is barred from group suggestion pools by a cited override.
- **Comp identity**: the planner names the playstyle a party is *becoming* — brawl, clap, kite, brawl-clap, **clap-kite** (a fifth playstyle the validation rounds identified: bomb from range, reset on cooldowns), or a **bomb squad** detachment — with per-member fit verdicts and named misfits.
...
A green unit test suite proves implementation behaviour, not that every recommendation is strategically correct. Blind validation rounds with experienced callers and real-comp evidence are the important external checks.

See `tests/VALIDATION.md` for the standing rules and the decision log, and `notes/validation/` for the full dated history.
...
```text
CLAUDE.md                      the contributor and tooling guide: invariants, the ONE gate list, build chain
MASTERSHEET.md                 tuning control surface (tune: blocks override everything)
...
pipeline/                      the engine's data layer
  README.md                    the pipeline, the patch workflow, the effect layer, the mechanics layer
...
tests/
  ...
  test_tone.py                 writing-conventions gate
  VALIDATION.md                the decision log (standing rules, index rows, pins); full text in notes/validation/
...
review/                        curation boards generated locally by pipeline/build_stat_chart.py,
                               build_magnitude_review.py and build_effect_review.py (gitignored)
```

## Recommended roadmap

The active track is the **role layer's remaining increments** (`roles-design.md`; the kit-doctrine advisor shipped as increment 2): forge role assignment with validated need profiles, and uptime economics. Beyond that, near-term product work is focused on making the engine more useful to an actual caller without prematurely teaching the score to imitate popularity:

1. ~~add a fight-chain explanation such as Engage → Clump → Pierce → Burst → Secure → Reset~~ — shipped: the planner grades the fight stage-by-stage in the playstyle's own sequence and ties the recommended pick to the stage it repairs,
2. ~~infer composition identity and detect internally conflicted rosters~~ — shipped as a descriptive layer: the planner names the playstyle a party is becoming and flags split identities; identity-aware *recommendations* wait on validation,
3. ~~surface negative recommendations and redundancy warnings~~ — shipped: every suggestion surface now carries a negative/redundant verdict from a signed decomposition of the exact pick marginal (the score's own terms, not a new penalty — a scoring-side redundancy term was tried and rejected), and the planner says when a pick or member closes no gap because the comp is saturated,
4. support locked players/slots and constrained reforging,
5. save player/guild weapon profiles,
6. ~~build partial-composition neighbours from the committed cohort sample~~ — shipped: the killboard strip shows the observed organization rosters most like the current selection, shared picks highlighted, display only,
7. ~~cluster recurring observed composition families~~ — shipped as recurring observed cores: anchor pairs that repeat across organizations and battles, with their frequently observed supporting cast, mined deterministically and shown display-only (whole-roster clustering was measured and rejected — the kill-feed baskets are partial observations, so distance clusters would reflect coverage, not comps),
8. add enemy-comp / counter-drafting analysis,
9. build blind-validation tooling before allowing empirical evidence to influence recommendation scoring.
