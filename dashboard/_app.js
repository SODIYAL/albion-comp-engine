...
/* the rows the ENGINE judges at the current size, never the raw template:
   a ramp row (anti_zone: none_until 14) is absent from ENG.reqs at small
...
/* Monoline UI icons (owner 2026-09-11, from the reference set: 24 grid,
   ...
   a different thing. */
const UI_ICONS = {
  lock: '<rect x="5" y="10.5" width="14" height="10" rx="2.5"/><path d="M8 10.5V7.5a4 4 0 0 1 8 0v3"/><circle cx="12" cy="15.5" r=".9" fill="currentColor" stroke="none"/>',
  unlock: '<rect x="5" y="10.5" width="14" height="10" rx="2.5"/><path d="M8 10.5V7.5a4 4 0 0 1 8 0v.8"/><circle cx="12" cy="15.5" r=".9" fill="currentColor" stroke="none"/>',
  replace: '<path d="M4 8h13M14 5l3 3-3 3M20 16H7M10 13l-3 3 3 3"/>',
  refresh: '<path d="M20 4.5v5h-5"/><path d="M18.4 14a7.5 7.5 0 1 1-1.8-7.8L20 9.5"/>',
  close: '<path d="M6 6l12 12M18 6L6 18"/>',
};
const ui = (name, s = 14) =>
  `<svg class="ui ui-${name}" viewBox="0 0 24 24" width="${s}" height="${s}" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${UI_ICONS[name]}</svg>`;
...
function memberPop(i, ctx){
  ...
  return `<div class="dm-pop" role="group" aria-label="Slot ${i+1} — ${nameOf(w)}">
    ...
    <div class="dm-actions">
      ...
      <button class="dm-act dm-lock${PROV[i] === "l" ? " on" : ""}" data-lock="${i}" aria-pressed="${PROV[i] === "l"}" title="${PROV[i] === "l" ? "unlock — a refresh may rebuild this slot" : "lock — a refresh keeps this slot"}">${ui(PROV[i] === "l" ? "unlock" : "lock", 11)}${PROV[i] === "l" ? "unlock" : "lock"}</button>
      <button class="dm-act dm-rp${REPLACE_OPEN === i ? " on" : ""}" data-replace="${i}" aria-expanded="${REPLACE_OPEN === i}" title="ranked replacements for this slot, scored into the rest of the comp">${ui("replace", 11)}replace</button>
      <button class="dm-act dm-refresh" data-refresh="${i}" title="keep ${nameOf(w)} (locks it) and rebuild every unlocked slot — the next-best comp each press">${ui("refresh", 11)}refresh rest</button>
      <button class="dm-act dm-x" data-remove="${i}" aria-label="Remove ${nameOf(w)}">${ui("close", 11)}remove</button>
    </div>
...
function replaceListHtml(i){
  const w = party[i];
  if (!REPLACE_OPTS.length)
    return `<div class="dm-replace"><span class="fn">no replacement clears the comp's constraints for this slot — the rest of the roster holds every minimum ${nameOf(w)} covers</span></div>`;
  const signed = v => (v < 0 ? "−" : "+") + Math.abs(v).toFixed(1);
  return `<div class="dm-replace" role="group" aria-label="Replacements for ${nameOf(w)}">
    <span class="fn">replace ${nameOf(w)} with</span>
...
function buildCompBoard(ctx){
  ...
  const fine = (id, w) => {
    ...
    const det = id ? (book[id] || null) : null;
    let primary = L ? L.primary : "";
    if (det && det.word && det.class !== "healer") primary = det.word;
  ...
  adv.members.forEach((m, i) => {
    ...
    c.ms.push({ i, role: fine(m.role, party[i]) });
  });
      ...
      ${c.ms.map(({ i, role }) =>
        `<div class="dm wf-dm ${roleCls(party[i])}${SHEET_OPEN === i ? " sheet-open" : ""}${PROV[i] === "l" ? " locked" : ""}"
          ...
          <span class="wf-ctl">
            ...
            <button class="wf-c wf-rp${REPLACE_OPEN === i ? " on" : ""}" data-replace="${i}" title="replace ${esc(nameOf(party[i]))} — ranked alternatives" aria-label="Replace ${esc(nameOf(party[i]))}">${ui("replace", 12)}</button>
            <button class="wf-c wf-rf" data-refresh="${i}" title="keep ${esc(nameOf(party[i]))}, refresh every unlocked slot" aria-label="Keep ${esc(nameOf(party[i]))} and refresh the rest">${ui("refresh", 12)}</button>
            <button class="wf-c wf-x" data-remove="${i}" title="remove ${esc(nameOf(party[i]))}" aria-label="Remove ${esc(nameOf(party[i]))}">${ui("close", 12)}</button>
          </span>
          ${memberPop(i, ctx)}
        </div>`).join("")}
...
function renderWarning(){
  ...
  if (FORGE_NOTE){
    ...
    if (FORGE_NOTE.unchanged)
      forgeBits += `<div class="warn"><span class="t">Forge</span>
        <span class="b"><b>Unchanged.</b> The forge found the same roster. The search is deterministic — the same locks, content, style and size always give the same answer. Lock a different member, swap a pick, or change the style or size to get a different comp.</span></div>`;
...
function applyForgeResult(r, keptStored, keptLoadouts, keptProv, note){
  ...
  FORGE_NOTE = Object.assign({ feasible: r.feasible, filler: r.filler, held: r.held }, note || {});
...
/* "Refresh the rest" (owner 2026-09-11): hold every LOCKED slot (and the
   ...
   signature (the one on screen included) and returns the best roster not
   among them; no randomness, both engines agree. A changed lock, content,
   style or size starts the list over. When nothing new is reachable the
   roster stays and the note says so. */
function lockSignature(locked, goal){
...
function refreshUnlocked(holdIndex){
  ...
  const key = p => ENG.rosterKey(p);
  const cur = party.slice();
  if (cur.length === forgeSize && !AVOID.some(p => key(p) === key(cur))) AVOID.push(cur);
  let r;
  ENG.setContent(CONTENT, forgeSize, STYLE);
  try { r = ENG.forge(forgeSize, locked, lockedCombos, undefined, undefined, lockedGears, AVOID); }
  finally { ENG.setContent(CONTENT, SIZE, STYLE); }
  if (r.exhausted){
    FORGE_NOTE = { feasible: true, filler: [], held: [], exhausted: true };
    REPLACE_OPEN = null; REPLACE_OPTS = [];
...
document.addEventListener("click", e => {
  ...
  if (forgeBtn){
    ...
    if (forgeBtn.id === "reforge"){ refreshUnlocked(null); return; }
    const goal = Math.min(PLAN(), HARD_CAP);
    const reforgeAll = false;
  ...
  if (rp){
    const ri = +rp.dataset.replace;
    if (REPLACE_OPEN === ri){ REPLACE_OPEN = null; REPLACE_OPTS = []; }
    else {
      REPLACE_OPEN = ri;
  ...
  const rpt = e.target.closest("[data-replaceto]");
  if (rpt){
    const si = +rpt.dataset.replaceat;
    const o = REPLACE_OPTS.find(x => x.weapon === rpt.dataset.replaceto);
    party[si] = rpt.dataset.replaceto;
    /* the option lands exactly as valued: its combo and doctrine kit
       (the same landing a forged slot gets); a locked slot stays locked —
       the user chose it — a forged one becomes the user's pick */
    COMBO[si] = o ? o.combo : null;
    LOADOUT[si] = undefined;
    if (o && o.kit && o.kit.length){
      const L = (LOADOUT[si] = { _eng: 1 });
      for (const k of o.kit){ const g = ENG.gear[k]; if (g && g.slot) L[g.slot] = k; }
    }
    loadoutPrefillGear(si);
    if (o) loadoutApplySpells(si, o.combo); else loadoutPrefill(si);
    PROV[si] = PROV[si] === "l" ? "l" : "m";
    REPLACE_OPEN = null; REPLACE_OPTS = [];
    FORGE_NOTE = null;
    hidePdashFly();
    sortPartyByRole();
  ...
  if (rfr){ refreshUnlocked(+rfr.dataset.refresh); return; }
