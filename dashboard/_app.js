  ...
  const cards = idxs.map(i => { const w = party[i]; return (
    `<div class="dm ${roleCls(w)}${SHEET_OPEN === i ? " sheet-open" : ""}">
      ...
      <div class="dm-pop" role="group" aria-label="Slot ${i+1} — ${nameOf(w)}">
        ...
        <span class="fn">${roleOf(w, COMBOS_CUR[i])} · ${signed(contrib[i])} fit${flag(i)}${ENG.isExcluded(w) ? ' · <b class="offcomp" title="owner rule: not a default large-group pick at this size — swap advice below">off-comp at size ' + SIZE + "</b>" : ""}${review[i] && review[i].redundant && !ENG.isExcluded(w) ? ' · <b class="redund" title="redundancy warning (display only, never a score change): valued as a pick into the rest of the party, this member closes no capability gap — its jobs are already covered without it">jobs covered without it</b>' : ""}</span>
...
function compBoard(){
  ...
  /* four MAIN columns only (owner 2026-08-27: "only show main roles like
     tank support dps and healers") — the fine played role stays in each
     member's hover. Order matches the roster's own role sort. */
  ...
  const cols = new Map();   /* class -> {name, color, ord, ms} */
