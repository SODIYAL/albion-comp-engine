...
**#1 — THE UNIT RE-FIT (blocked on two owner decisions; see "Current engine model" above for the full statement).** Every target and soft cap is in weapon+spell-pick units; supply is measured on whole dressed people. Nothing else on this list matters as much, because both charts and the whole coverage half of fitness are currently comparing two different measurements. The work itself is mechanical once ruled — re-fit every row from the same real comps read as people, rebuild, run the full battery, confirm real comps land between the tick and the rim. What is NOT yet decided: (a) the evidence bar (which comps' gear records are complete enough to set a number — blap is 71.7% kit fill and would drag targets down for a recording gap, not a real one); (b) what a comp with no kit set should read (person-sized numbers make a naked roster read catastrophically low — correctly, but unusably — so the app likely assumes the seat's doctrine kit when none is set). Do not start it piecemeal.
