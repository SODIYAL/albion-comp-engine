...
Layout (owner passes 2026-08-23, 2026-08-27, then the 2026-09-02 density redesign above): the forge honesty reports (`#warn-slot`) sit directly under the wheel card in `.main`, placed by `_layout.css` — never hidden. The old right-hand fitness / weakness / recommendation stack is intentionally hidden. The full capability board remains the deep diagnostic layer on the same ceiling ruler as the radar.
