...
py -3 tests/test_golden.py             # must stay 13/13
py -3 tests/test_js_parity.py          # app JS scoring == engine.py (needs node)
...
- More content templates (Hellgate 5v5 and Roads 7 from the MVP list are
  still missing) and expert adjudication of the two 20-size templates.
  Phase 3 battle-data ingestion for MetaPrior. Usage sample is still small
  (24 battles / 359 players) — re-run at ~200 battles (VALIDATION V7).
