...
- **The product page exists: `app/index.html`** (generated by
  `pipeline/build_app.py`). Content picker, party builder, fitness + floor
  alarms + gaps + ranked next-picks with reasons; state in the URL hash for
  sharing. Scoring runs in the browser via `pipeline/app_scoring.js` — a
  port of `engine/engine.py` that `tests/test_js_parity.py` holds equal
  (change one, change both, rerun parity).
