...
This file is the current-state handoff. `MASTERSHEET.md` is the LIVE expert control surface — its `tune:` blocks override everything at build time, so read it first for what the engine actually uses. `tests/VALIDATION.md` is the ruling index (standing rules, one line per ruling with pin and archive location, open questions); the full append-only log — every dated round, every owner quote, every score — is `notes/validation/` by date range. `albion-comp-engine-design.md`, `MECHANICS_TODO.md` and `pipeline/README.md` hold the design history, the mechanics backlog and the patch workflow. `CLAUDE.md` carries the load-bearing invariants and the one authoritative gate list. Consult them when changing mechanics, calibration, provenance, or validation. History that used to live here (PR archaeology, shipped-roadmap narrative, harvest counts) was removed 2026-09-07; `git log` and VALIDATION.md have it.
...
- **Calibration**: the targets remain conservative (good comps over-cover ~1.8×); sharpening needs expert blind rounds (the train / validation / holdout standing rule in `tests/VALIDATION.md`). Watch items: Hellfire under clap_kite, castle-25's saturated tail quality.
