# Castle Outpost Comp Book — design (2026-08-27)

**Status:** DROPPED 2026-09-07. Decided section by section on 2026-08-27,
never built; the implementation plan was deleted the day it was dropped.
Kept as the design record for two ideas nothing else in the repo covers —
content-labelled fights and a ratified comp book (Decisions 2 and 4). The kit
half (Decision 5) was overtaken by the 2026-09-04 party harvest
(`pipeline/sample_parties.py`: killer-party rosters with full gear from the
official API), which feeds the kit doctrine and the style × size rows today.

**Decisions embedded:** evidence-first clustering; all four evidence
sources; correctness = comp + per-seat kits + budget tiers; vertical slice
(castle outposts first); doctrine-coherence tests; stage gates between
every step.

## Problem

Extend the engine so it can answer, with evidence, "what comps exist in
castle outposts (7-man) and what should each player wear" — as the first
vertical slice of a five-content program (castle outposts, castle fights,
territory battles, faction warfare, blackzone roam). Every later content
reuses this slice's machinery; none of it is built content-specific except
the data.

End state of the slice:
...
3. Clusters are ratified in validation rounds; ratified kinds land in the
   curated `pipeline/comp_book.yaml` (evidence-cited, release-blocking).
...
5. Forge generation at castle_outpost can target a ratified comp kind through
   ...
   budget tiers.
6. Gates exist per step (see Tests): contract tests per artifact, fail-closed
   builders, doctrine-coherence tests, golden pins from validation rounds, parity.
...
identify outpost fights reliably is unverified. The scouting step in
Decision 2 answers it cheaply; if segmentation proves weak, the fallback is
caller-confirmed labeling of mined candidate fights.

## Decisions

### 1. Approach: extend existing machinery end-to-end

Approach A (chosen at design time): every stage reuses a pattern the repo
already trusts — sampler discipline from `sample_rosters.py`, mining honesty
from `build_cohort_families.py`, curated constants from `roles.yaml` need
profiles (curation judgment), generation-only gating from the forge
predicate channel, and the validation-round loop from VALIDATION.md. The
comp book display surface absorbs what "approach B" (book-first) would have
shipped, as the display layer of stage 3. Rejected: folding evidence into
template retuning (hides comp kinds inside weights; collides with the
anti-circularity rule).

### 2. Evidence layer: outpost-segmented sampling
...
present; if not, the fallback above activates.
...
ingesters via a committed mapping table from their activity tags to the
engine's contents; caller-submitted comps are ordinary `published_comp`
records.

### 3. Clustering: `pipeline/build_comp_clusters.py`
...
**Honesty gates (in the spirit of cohort families):** minimum
...
Published-comp sources do not enter the clustering math; they are shown
beside clusters at ratification so intent-labeled and observed evidence stay
distinguishable.

### 4. Ratification: validation rounds and `pipeline/comp_book.yaml`
...
2. The caller's call comes first: name / merge / split / kill. The call is
   logged in VALIDATION.md before the engine's label is revealed.
3. Disagreements convert same-day into a rule, an override, or a golden
   pin. A mislabeling by the engine is a finding about the style/identity
   layer — a hypothesis for a maintainer decision under the anti-circularity
   rule, never an auto-retune.
...
explicit `basis: curation_judgment`. Book entries never touch scoring
weights; machine consumers are the forge's generation targets and the
display layer.

### 5. Kits and budget tiers: the content dimension
...
- Named bands per book entry (e.g. budget/standard/prime) with approximate
  IP ranges and gear-tier guidance — curated constants informed by evidence
  (curation judgment), the need-profiles pattern.
...
call vs the caller's is the ongoing validation-round signal.
...
- `pipeline/out/content_rosters.json` — per roster: content label, weapon
  list, per-player equipment (chest/head/shoes/cape) + average IP where
...
- `pipeline/out/comp_clusters.json` — deterministic ordering; battle ids kept
  for audit; org identifiers never in the page embed. Display/evidence only —
  nothing in scoring reads it.
- `pipeline/comp_book.yaml` (curated constants, roles.yaml mold):

```yaml
castle_outpost:
  - id: outpost_dive
    name: "Dive squad"
    decided: 2026-09-01        # (example) decision date; must match a VALIDATION.md round
    ...
    kits: ...                  # per-seat kit doctrine refs (Decision 5)
    budget: ...                # IP/cost band refs (Decision 5)
    notes: "doctrine commentary"
```

## Engine changes

- The book matcher in `comp_identity` (descriptive, golden-pinned to leave
  fitness untouched).
- `suggest_pool` / forge predicate channel: seat bands and per-seat pools
  from a selected book entry; `off_book` flag in swap review.
- `kit_options` content parameter with an explicit thin-evidence fallback.
- Both ports: every change lands in `engine/engine.py` +
  `engine/app_scoring.js` with parity cases at 1e-9.

## Tests and gates

### Contract tests (script-style, exit 0 = pass)

- `tests/test_content_rosters.py` — step-1 gate: labels only from the
  committed zone table, unlabeled never guessed, wiped-side rule enforced,
  LF and determinism on offline re-analysis, `_meta` completeness (schema
  version, zone-table hash, bias notes, counts).
...
- `tests/test_comp_book.py` — schema; weapon keys/seats/styles resolve
  ...
  dangle-checked; `basis: curation_judgment` required when uncited;
  `decided:` date cross-checked against a VALIDATION.md round; LF discipline.
...
- New golden cases in `tests/test_golden.py` — one per validation-round
...
- `tier2_blindtest` gains castle_outpost leave-one-out cases once enough
  labeled comps exist. This may trail the slice — "not enough evidence yet"
  is a reported state, never a fudged pass.

### Doctrine-coherence tests (`tests/test_doctrine.py`)
...
are judgment calls → curated constants captured during the ratification
validation rounds (curation judgment), cited in the test file, never invented. Until a floor
is decided, its case runs report-only (prints the measure, does not gate);
findings become validation-round material under the anti-circularity rule.

### Stage gates: each step blocks the next
...
3. **Clusters → book:** `build_dataset.py` resolves every book evidence
   ...
   or precede its evidence. Ratification is process-gated: `decided:` dates
...
## Standing invariants as acceptance criteria
...
- Anti-circularity: gate findings are hypotheses; template or style retunes
  the evidence suggests need a maintainer decision.
...
4. First validation round → initial `comp_book.yaml` + `test_comp_book.py` +
...
8. `test_doctrine.py` (report-only where floors are undecided → decided
   floors land as gates in follow-up validation rounds).

Steps 2–3 were useful standalone even if later steps slipped: labeled
evidence accumulates from the moment step 2 lands.

## Deferred / dropped

The whole slice was dropped 2026-09-07. What survives elsewhere: the party
harvest (`sample_parties.py`) supplies observed kits and the style × size
rows; content-labelled fights and a ratified comp book remain unbuilt ideas
recorded here.
