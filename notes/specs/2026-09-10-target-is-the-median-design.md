# Target is the median — design (2026-09-10)

Status: implemented 2026-09-10 (branch `target-median`; plan
`notes/plans/2026-09-10-target-is-the-median.md`). Deviations from the
draft, each recorded in `notes/validation/2026-09b.md` "Target is the
median": (1) a FOUR-STAGE board was added mid-build (red below the bare
minimum for winning, orange above it but not yet ideal, green at ideal,
purple too much), so every row also carries `min` = p10 / least fitted
comp, exposed as `target_min()`; (2) the content re-fit also RAISES a soft
cap to 1.15 x the most any comp fielded where a comp exceeded the old cap
(several medians sat above their caps) and never lowers one; (3) the
descriptive lenses moved onto the two lines — kill lights bar on the
minimum, chain stages grade weak < min <= ok < typical <= strong,
`redundant_gain_max` 0.05 -> 1.0; (4) the pooled `balanced` cell is coded
but the committed board predates it (BACKLOG: regenerate on the harvest
machine). Open: T25b pierce (BACKLOG, maintainer decision). Decision
implemented: targets come from the harvest median (2026-09-10). The
motivating case: three healers in a party of 15 is normal and standard to
aim for, and the board read it as too many.

## Problem

The template numbers are meant to be targets to hit for the style of comp,
and every real comp overshoots them. A board reading heal burst / heal
sustain at 12/4 tells a reader the party has too many healers, when three
healers in a party of 15 is normal.

Every `target` in the model was the **left edge** of what winning parties
field: the harvest convention was `target = 0.9 × p10` (90% of winners field
at least this much), and the hand-fitted content rows followed the same
...
- The board read `heal_burst 12 / 4.6` for a normal 15-man kite with
  three healers. Every row overshot; the label carried no signal.
- The forge agreed with the label: one healer "covered" 15 people and a
  second/third healer was worth ~0.3 points. The pick card contradicted
  itself ("already covers sustained healing … Hallowfall closes that").
- The measured **median** (p50) — where winners actually cluster — was
  computed in the evidence board, written into the YAML comments, and
  used by nothing.

This was one semantic error inherited by all 23 capability rows, every
...
### Harvest layer — `pipeline/derive_style_bands.py`
...
- **A `balanced` band, pooled.** Before this change `balanced` never read a
  band (2026-09-04 rule, made when the bands were floors and the content
  rows were the safer default). With medians the harvest is the better
  source at every size it can see. `audit_style_rosters.py` writes one
  extra cell per band, `balanced|<band>`, aggregated over EVERY roster in
  the band regardless of style label (labelled and unlabelled alike — the
  pool is "winners at this size", not "winners with a name"). Same
...
- Soft cap ≤ target rows are still skipped.

The expected shape of the change, from the 2026-09-09 board (kite, ref 17,
per-person scaled to 15): heal_burst 4.6 → 9.1, heal_sustain 4.9 → 10.0,
cleanse 1.6 → 5.3, tankiness 34.9 → ~42.

### Content rows — the six templates
...
The 10/14/15/19-man `zvz_*` comps are unmapped to any content and stay so
(they feed the blind-test gate, not the fit). Whether to map them is a
separate decision and is not needed here: at 10+ the harvest medians carry
the targets.
...
`content` vs `content_min` below. `scales`, `weight`, `ramp`, `optional`,
...
`set_content` records per capability where the effective target came
from: `harvest` (band row target), `harvest_borrowed` (a thin cell
borrowing its nearest), `content` (content row, median-fitted),
`content_min` (content row still on the old minimum — thin or no comps).
Exposed as `target_source(cap)` on both engines; display provenance only,
never scored.

## Engine changes — `engine/engine.py` and `engine/app_scoring.js`
...
- **`target_source(cap)`** as above; carried by parity like every other
  descriptive read. The dashboard uses it to label rows.
- The floor clamp ("a hard floor never exceeds the target it guards")
  keeps working; with higher targets it binds less often.

### Dashboard — `dashboard/_app.js`, `_decision_layer.js`

- The capability board label reads `have / typical` (heading "CAPABILITY
  SUPPLY VS. TYPICAL WINNER"; the exact copy is a build-time string). The
  ring keeps its tick at the target and its end at the soft cap — that IS
  the band "winners field target..cap".
...
- Golden cases that flip are hypotheses, one decision each, collected in a
  validation round (the caller's pick first, the engine's after). Expected
  flips: parties where the forge previously stopped at one healer / one
  tank because the floor was "covered".
...
- `tests/test_forge.py`: the pins above (F30a–f); existing pick-score
  invariants must hold unchanged.
- `tests/test_golden.py`: re-run; every flip logged and decided before the
  pin moves.
- `tests/test_js_parity.py`: `target_source` added to the parity payload.
- `tests/test_dashboard_layout.py`: board label / chip contract (L20).
- A derive test (`tests/test_style_bands_derive.py`, script-style like the
  rest): on a fixture evidence board, `derive_style_bands` writes `target
  == p50`, writes soft-cap-only when `p50 == 0`, emits a `balanced` cell,
  and no longer reads `zero_share`.
- `pipeline/evidence_lint.py`, `test_provenance.py` (byte-identical
  rebuild after the YAML changes), `test_validation_modes.py`.

Records: `tests/VALIDATION.md` carries one index row "Target is the median
(2026-09-10)" pointing at the dated log in `notes/validation/2026-09b.md`
with the before/after table. Anti-circularity: the harvest that fitted the
bands is not re-tuned against gate results; gate findings are hypotheses
for a maintainer decision. `MASTERSHEET.md` states the convention line
("targets are harvest medians; soft caps 1.15 × p90") beside the existing
`tune:` knobs. `HANDOFF.md` "The engine today", `pipeline/README.md` (style
bands paragraph, content rows paragraph), the `derive_style_bands.py`
docstring and `BACKLOG.md` (item closed; follow-up for the roads / castle /
faction_war comp gaps) carry the same rule.
...
- Any change to the scoring curve's shape, headroom, or over-stack
  ...
  or the forge over-eager, that is a separate decision with its own
  evidence.
- The pooled `balanced` cell in the committed board (regenerate on the
  harvest machine; BACKLOG).
