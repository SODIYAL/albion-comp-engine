...
Findings record, generated by `pipeline/compare_fold.py` after the fold. Report only: nothing in the build reads it; every moved row is a hypothesis for a maintainer decision, never a retuning input (anti-circularity).
