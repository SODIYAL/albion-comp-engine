# Seat Pooling for Thin Slots — implementation plan (2026-09-08)

Status: implemented 2026-09-08 (R34a/R34b; VALIDATION.md index 09-08
"Seat pooling"). The checkboxes below are the plan as written; every task
is done.
...
**Spec:** `notes/specs/2026-09-08-coherent-style-kits-design.md`, Decision 3.
...
### Task 1: Ship the pools — done (R34a)
...
**Interfaces:** on every seat record that has `kit`: `kit_pool: {slot: [[item, players], ...]}` (sorted by players desc, id asc; items with >= 2 players; slots head/shoes/cape/potion/food) and `kit_by_chest: {chest_id: {slot: [[item, players], ...]}}` (same slots, chest = the build's own normalized, uniform-legal chest). The gang band writes the same keys under `kit_bands.gang`. Style cells write none.

- [x] Test R34a: every seat with `kit` carries `kit_pool` with only those five slots and integer player counts >= 2, `kit_by_chest` keyed by catalog chests of the seat's uniform classes (or a weapon's extension), and no style cell carries either key.
- [x] Run roles, see R34a fail, implement in the killboard block (collect per seat: `pool_players[slot][item] = set(players)` and `chest_players[chest][slot][item] = set(players)` over `kept` builds of member weapons whose chest passes `seat_ok`/`weapon_ok`), write after `tgt["kit"]`, skip when `style is not None`.
- [x] Rebuild, roles green, commit.

### Task 2: The thin rule in both ports — done (R34b)
...
- [x] Test R34b (mechanism on the dataset): find a seated weapon at 20 balanced whose head weapon-tier modal has < 5 votes and whose seat `kit_by_chest[chest of its kit][head]` top item has >= 5 players and differs from the thin item; `kit_options(w)["kit"]["head"]` is the pool item with `pooled == "seat|chest"`. A weapon whose head modal has >= 5 votes keeps its own (no `pooled`). A thin potion goes to `kit_pool["potion"]` top with `pooled == "seat"`.
- [x] Run roles, see R34b fail, implement: in the slot loop before `options[slot] = ranked[:top_n]` is sliced, compute `thin = not wslot or max(wslot.values()) < POOL_MIN_VOTES`; if `thin and slot in POOLED_SLOTS and role is not None`: candidates = (`kit_by_chest[chest][slot]` if slot in (head, shoes, cape) and chest known) then `kit_pool[slot]`; the first `[item, players]` with `players >= POOL_MIN_VOTES` and `item` present in `ranked` moves to the front with `pooled`/`pooled_n` set. `chest` = `options["armor"][0]["gear"]` when armor was already ranked (sorted slot order puts armor first), else None.
- [x] Mirror in JS; rebuild dashboard; parity 60/60; roles green; commit.

### Task 3: Audit floor, display, docs, gates — done
...
- [x] R24: add the `who` player sets and `continue` when the modal has < 5 players; roles green.
- [x] Loadout: prefill records `L._pooled = {slot: source}`; the engine-kit line appends "helmet/boots from the seat's <chest> wearers" style text; cleared with `_eng`.
- [x] Docs + VALIDATION entry with the measurements from the spec (tiles by evidence source before; bootstrap match rates) and the after-numbers (tiles by source).
- [x] Full gate list (14, cohort from PowerShell), commit, push, update the PR body.
